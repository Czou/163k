...
function getNewPage(NewPageNo){
	var iskill = $('#iskill').val(),
		...
		colname = $('#colname').val(),
		payid = $('#payid').val(),
		keystyleid = $('#keystyleid').val() || 0,
		keyword=$('#keyword').val() || '';
	var _keyValJson={"action":"pagemyorder","isadmin":"1","iskill":iskill,'skill':skill,'startdate':startdate,'enddate':enddate,'colname':colname,'payid':payid,"keystyleid":keystyleid,"keyword":keyword,"PageNo":NewPageNo};
