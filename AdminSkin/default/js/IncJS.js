...
function addquestion()
{
	...
	var url="admin_qiang.aspx?action=add&ischeck="+ischeck+"&question="+encodeURIComponent(document.getElementById('question').value)+"&answer="+encodeURIComponent(document.getElementById('answer').value);
...
function showwapid(_id)
 {
	...
	var	arrstr =  new Array("0|网站首页专用","1|通用:全站有效","2|_信息频道","3|_房产频道","4|_团购频道","5|_活动频道","6|_人才频道","7|_投票系统","8|_购物频道","9|_店铺频道","10|_视频频道","11|_文章/新闻","12|_网址频道","13|_礼品兑换","14|_图库频道","15|_黄页频道");
...
function arrtemplatego(_id){if(_id == ""){show_val('arrtemplate','default');}else{show_val('arrtemplate',_id);}}
 function lableidgo(_id){
  ...
  else if(_id == "14"){var	arrstr =  new Array("0|::请选择::","13001|图库首页","13000|图库通用标签","13010|••图库列表","10111|___图库内容页");}
...
function DeleteSignRow(rowid){
	...
	up_blur(this,0,'urlhidden',i_url)
}
...
function set_newimage(mapdomid){
	if(!mapdomid || typeof 'undefined' === mapdomid){
		mapdomid='maparea';
	}
	var city="";
	try{
	...
	var shop_x='';
	if(document.getElementById('shop_x')){
		shop_x = document.getElementById('shop_x').value;
	}
	var shop_y='';
	if(document.getElementById('shop_y')){
		shop_y = document.getElementById('shop_y').value;
	}
	var shop_z='';
	if(document.getElementById('shop_z')){
		shop_z = document.getElementById('shop_z').value;
	}
	var chraddress = '';
	if(document.getElementById('chraddress')){
		chraddress = document.getElementById('chraddress').value;
	}
	var sysset = '';
	if(document.getElementById('sysset')){
		sysset=document.getElementById('sysset').value;
	}
	var ggmap = 0;
	if(document.getElementById('ggmap')){
		ggmap=document.getElementById('ggmap').value;
	}
	
 	var url='../ezmarker/index.aspx?mapdomid='+mapdomid+'&city='+encodeURIComponent(city)+'&chraddress='+encodeURIComponent(chraddress)+'&x='+shop_x+'&y='+shop_y+'&z='+shop_z+'&ggmap='+ggmap+'&sysset='+sysset;
	...
	{ 
		...
		function requestdataquyu1()
		{
			if(xmlhttp.readyState==4)
			{
				if(xmlhttp.status==200)
				{
					 BuildSel_tt(xmlhttp.responseText ,document.getElementById("classid"),"请选择图集")
				}
			}
		}
	} 
	...
	{ 
		...
		function requestdataquyu2()
		{
			if(xmlhttp.readyState==4)
			{
				if(xmlhttp.status==200)
				{
					BuildSel_tt(xmlhttp.responseText ,document.getElementById("classid1"),"请选择图库")
				}
			}
		}
	} 
...
function newphotosetclick(aa,bb){
	if(parseInt(aa)==parseInt(bb.value)){
		...
		if(aa=="2")
		{
			$("photostyleid").value=aa;
			//$("SignFrame").style.display="none";
			$("upload_way_2").style.display="";
			$("upload_way_1").style.display="none";
		}
		else
		{
		//	$("SignFrame").style.display="";
			$("upload_way_1").style.display="";
			$("upload_way_2").style.display="none";
...
function newphotosetclicknews(aa,bb){
	i_url= {};
	i_urlmark= {};
...
function setclick2(aa,bb){
	var styleid = document.getElementById(bb);
	styleid.value=aa;
	if(aa=="2"){
		document.getElementById('urlhidden').value = '';
		document.getElementById('urlmark').value = '';
		document.getElementById('upload_way_2').style.display="";
		document.getElementById('upload_way_1').style.display="none";
		document.getElementById('upload_way_0').style.display="none";
		return false;
	}else if(aa=="1"){
		document.getElementById('urlhidden').value = '';
		document.getElementById('urlmark').value = '';
		document.getElementById('upload_way_1').style.display="";
		document.getElementById('upload_way_2').style.display="none";
		document.getElementById('upload_way_0').style.display="none";
	}else{
		document.getElementById('urlhidden').value = '';
		document.getElementById('urlmark').value = '';
		document.getElementById('upload_way_0').style.display="";
		document.getElementById('upload_way_2').style.display="none";
		document.getElementById('upload_way_1').style.display="none";
	}
	var tt =parseInt(document.getElementById("count").value);
...
function addphotorownews(){
	...
	var newNameTD=newTR.insertCell(0);
	//添加列内容
	newNameTD.innerHTML = "<input type=\"text\" name=\"url_"+ab+"\" onBlur=\"up_blur(this,"+ab+",'urlhidden',i_url)\" style=\"width:320px;\"  value='http://' /> 描述：<input type=\"text\"  style=\"width:200px;\" name=\"urlmark_"+ab+"\" onBlur=\"up_blur(this,"+ab+",'urlmark',i_urlmark)\" /> <img  style=\"cursor:pointer\" onclick=\"DeleteSignRow2('"+ab+"',i_url,i_urlmark)\" src='../adminskin/default/images/del.jpg' border=0>";
	//将行号推进下一行
	$("count").value = (rowID + 1).toString() ;
	$("total").value = parseInt($("total").value)+1;
}
...
function moni_up_blur(rowid,obj1,obj2){
	var i_txt = '',k_txt = '';
	var urlhidden = document.getElementById('urlhidden');
	var urlmark = document.getElementById('urlmark');
	delete obj1[rowid];
	delete obj2[rowid];
	for(var i in obj1){
		i_txt+=obj1[i]+',';
	...
	i_txt = i_txt.slice(0,i_txt.length - 1);
	k_txt = k_txt.slice(0,k_txt.length - 1);
	urlhidden.value = i_txt;
	urlmark.value = k_txt;
	
}
function DeleteSignRow2(rowid,obj1,obj2){
	var signFrame = findObj("SignFrame",document);
	...
	if(document.getElementById("total")){
		document.getElementById("total").value = parseInt($("total").value)-1;	
	}
	
	moni_up_blur(rowid,obj1,obj2);
}
...
function checkaddlive(o){
	...
	if($("#chrtel").val()=="" && $("#chrmobile").val()==""  && $("#chrqq").val()==""){
		alert("对不起,联系手机,联系电话,联系QQ必填一项!");
		$("#chrtel").focus();
		return false;
	}
	var if_isindex = $('input[name="isindex"][value="1"]').prop('checked');
	if(!!if_isindex){
		
		if($("#isindexdate").val()==""){
			alert("对不起,请输入首页头版有效期");
			$("#isindexdate").focus();
			return false;
		}
	}
	var if_istop = $('input[name="istop"][value="1"]').prop('checked');
	if(!!if_istop){
		if($("#istopdate").val()==""){
			alert("对不起,请输入大类置顶有效期");
			$("#istopdate").focus();
			return false;
		}
	}
...
function getKeyWordShop()
{
    ...
    getobject("keytishi").style.top = (top + 25) + "px";  //设置提示层的位置,上
    getobject("keytishi").style.display = "block";  //设置提示层可见
    var url = "../request.ashx?action=shop_small&keyword="+encodeURIComponent(getobject("xiaoqu").value)+"&ishtml=1&tt="+encodeURIComponent(Digital);
	jQuery.ajax({'url':url,success:function(data){
		if(data.islogin === '1'){
			jQuery('#keytishi').html(data.MSG);
		}else{
			jQuery('#keytishi').html(data);
		}
	}});
	//模板路径shop/shop_small_div_list.html
}
...
function show(_showobj ,_showt){document.getElementById(_showobj).style.dispaly="";document.getElementById(_showobj).style.display='';if( _showt != "" && _showt != null ){document.getElementById(_showobj).innerHTML=_showt;}}
function show_val(_showobj ,_showt){$(_showobj).value=_showt;document.getElementById(_showobj).value=_showt;}
function hide(_showobj){if( _showobj != "" && _showobj != null ){document.getElementById(_showobj).style.dispaly='none';document.getElementById(_showobj).style.display='none';}}

function showIntorder(action,sid,key){
	var url = '../request.ashx?action=categorymove&id='+sid+'&key='+key+'&isadmin=1&pn='+action;
	var Digital=new Date();
	Digital=Digital+40000;
...
function delCategory(sid,key){
	if ( !confirm("你的删除操作将会影响到已经使用该分类的正常显示，是否强制删除？")){return false;}

	var url = '../request.ashx?action=categorydel&id='+sid+'&key='+key+'&isadmin=1';
	var Digital=new Date();
	Digital=Digital+40000;
	url=url+"&k="+encodeURIComponent(Digital);
	jQuery.ajax({url:url,success: function(data){
...
function hideCategory(action,sid,key){
	var url = '../request.ashx?action=categoryisopen&id='+sid+'&key='+key+'&isadmin=1&isopen='+action;
	var Digital=new Date();
	Digital=Digital+40000;
	url=url+"&k="+encodeURIComponent(Digital);
	jQuery.ajax({url:url,success: function(data){
...
function showTgCategory(str,docId,nowcc){
	var url="../request.ashx?action=category&id=17";
	var  Digital=new  Date();
	Digital=Digital+40000;
	url=url+"&k="+encodeURIComponent(Digital);
	var data;
...
function showPPCategory(pdid,str,docId,nowcc){
