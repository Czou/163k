...
var error="";
var shopcomment_error_name=error+"对不起,请填写您的用户名！";
...
var user_length_error=error+" 用户名长度错误,3-15字符内！";
var user_regist_ok="<font color=\"green\">恭喜您，该用户名还未被注册，您可以使用这个用户名！";
var user_regist_error=error+" 该用户名已经被注册，请选用其他用户名。";
...
var email_nothing_error=error+"请输入您的电子邮件地址";
...
function checkregister(){
	...
	if(document.form1.chrname.value==""){
		errorFunction(document.form1.chrname,user_length_error);
	...
	if(document.form1.telmust.value=="*"){
	    ...
	    if(checkmobile(strtel) && strtel.length>0){
		   ...
		   alert(chkchrtel_nothing_error)
		   return false;
		}
	}
	trimspace(document.form1.chrqq);
	if(document.form1.qqmust.value=="*"){
		var chrqq=document.form1.chrqq.value;	  
	    if(!chrqq.length>0){
		   alert(chrqq_nothing_error)
		   return false;
		}
	}
	 ...
	 if(document.form1.styleid.value=="2"){
		...
		var chrtruename=document.form1.chrtruename.value;	   
		if(chrtruename=="" ){
		   alert(chkchrtruename_nothing_error)
		   return false;
		}
		trimspace(document.form1.qiyedizhi);
		var qiyedizhi=document.form1.qiyedizhi.value;	   
		if(qiyedizhi=="" ){
		   alert(chkqiyedizhi_nothing_error)
		   return false;
		}
	...
	addregister(document.form1.chrname.value,document.form1.chrpwd.value,document.form1.chrpwd_1.value,document.form1.chremail.value,document.form1.code.value,document.form1.chrqq.value,detailsid,answer,chrtruename,qiyedizhi,faren,document.form1.chrtel.value,document.form1.styleid.value);
	return false;
}
...
function addregister(chrname,chrpwd,chrpwd_1,chremail,code,chrqq,detailsid,answer,chrtruename,qiyedizhi,faren,chrtel,styleid)
{
		...
		var jqxhr = $.getJSON(url,function(data){
			...
			if(d.islogin === '1'){
				...
				window.location.href= 'checkusers.aspx?action=s2&name='+encodeURIComponent(chrname);
			}
			else if(d.islogin === '3'){
				alert('恭喜您，注册成功了！您好，您的账户需要短信激活才能登录！');
				window.location.href= 'checkusers.aspx?action=s3&name='+encodeURIComponent(chrname);
			}else{
				$('#have_login').hide();
				alert(d.error);
			}
