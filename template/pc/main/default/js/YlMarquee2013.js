...
var ul=$("ul",div);
var tli=$("li",ul);
var liSize=Math.ceil(tli.size()/o.col);
if(o.direction=="left")
tli.css("float","left");
...
if(liSize>=o.visible){
	ul.append(tli.slice(0,o.visible*o.col).clone()) //复制前o.visible个li，并添加到ul的最后
	li=$("li",ul);
	liSize=li.size();
	...
	ul.hover(function(){MyMarout&&clearTimeout(MyMarout);MyMar&&clearInterval(MyMar);},function(){MyMar=setInterval(ylMarquee,o.speed);}
	);
};
function ylMarquee(){
	if(o.direction=="left"){
	...
	if(o.direction=="up"){
		...
		if(div.scrollTop() % (liHeight*o.visible) == 0){
			div.scrollTop()>=ulHeight && div.scrollTop(0);
			MyMar&&clearInterval(MyMar);
			MyMarout = setTimeout(function(){MyMar=setInterval(ylMarquee,o.speed);},3000);
		}
	}
