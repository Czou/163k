...
function is_login(siteUrl,tplPath,hasQQ){
	...
	$.getJSON(url,function(data){
		...
		if(data[0].islogin==="1"){
			...
			$('#is_login_fail').hide();
			$('#is_login').show();
			$('#header').addClass('header showDuihuan');
...
function Login(str1,str2,str3)
{
	...
	if(jump_url =='' || jump_url == null){
		...
		jump_url = 'member/userindex_s.aspx';
	}else{
...
function getQuan(str){
	...
	$.ajax({url:url,success:function(data){
		data = eval('('+data+')');
		if(data.islogin === '1'){
			inner.empty().append(data.message+'<a href="#" onClick="return duihuan();" class="btn dh_btn2">确认兑换</a>');
	...
	if(hasWrongPW){
		...
		$.getJSON(url,function(data){
			if(data[0].islogin === '1'){
				...
				quan_num.val('');
				//initPage();
				$('#results_node').prepend(data[0].record);
