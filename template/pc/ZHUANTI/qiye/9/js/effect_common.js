...
function Show_TabADSMenu(tabadid_num,tabadnum,tabNums){
	...
	for(var i=0;i<tabNums;i++){
		if(document.getElementById("tabadmenu_"+tabadid_num+i))
			document.getElementById("tabadmenu_"+tabadid_num+i).parentNode.className="";
	}
	document.getElementById("tabadmenu_"+tabadid_num+tabadnum).parentNode.className="selected";
	document.getElementById("tabadcontent_"+tabadid_num+tabadnum).style.display="block";
}

function Show_TabADSMenu3(tabadid_num,tabadnum,tabNums)
{
	for(var i=0;i<tabNums;i++){
		if(document.getElementById("tabadcontent_"+tabadid_num+i))
			document.getElementById("tabadcontent_"+tabadid_num+i).style.display="none";
	...
	document.getElementById("tabadcontent_"+tabadid_num+tabadnum).style.display="block";
	//alert(document.getElementById("tabadmenu_"+tabadid_num+tabadnum).parentNode.className);
}
/*选项卡切换
对象属性{选项卡序号|按钮序号||选项卡总数量}
*/
...
function dk_slideplayer(object,config){
	...
	this.factory = function(){
		jQuery(this.obj).css({position:"relative","text-align":"left","font-family":"Arial",zIndex:"0",margin:"0",padding:"0",width:this.config.width,height:this.config.height,overflow:"hidden"})
